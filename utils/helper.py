...
def get_computers(objectSid, ldap_session, domain_dumper):
    computers = []
    ldap_session.search(domain_dumper.root, f'(&(objectcategory=computer)(mS-DS-CreatorSID={objectSid}))', 
            attributes=['sAMAccountName'])
    for computer in ldap_session.entries:
        computers.append(str(computer['sAMAccountName']))
    return computers

def get_spn(samname, ldap_session, domain_dumper):
    ldap_session.search(domain_dumper.root, '(sAMAccountName=%s)' % escape_filter_chars(samname), 
...
def clear_spn(samname, ldap_session, domain_dumper):
    ldap_session.search(domain_dumper.root, '(sAMAccountName=%s)' % escape_filter_chars(samname), 
            attributes=['SAMAccountName', 'servicePrincipalName', 'dnsHostName', 'msds-additionaldnshostname'])
    try:
        et = ldap_session.entries[0]
        ldap_session.modify(et.entry_dn, {'servicePrincipalName':[(ldap3.MODIFY_REPLACE, [])]})
    except IndexError:
        logging.error('Machine not found in LDAP: %s' % samname)
        return False    


def add_spn(samname, spns, ldap_session, domain_dumper):
    ldap_session.search(domain_dumper.root, '(sAMAccountName=%s)' % escape_filter_chars(samname), 
            attributes=['SAMAccountName', 'servicePrincipalName', 'dnsHostName', 'msds-additionaldnshostname'])
    try:
        et = ldap_session.entries[0]
        for spn in spns:
            ldap_session.modify(et.entry_dn, {'servicePrincipalName':[(ldap3.MODIFY_ADD, [spn])]})
    except IndexError:
        logging.error('Machine not found in LDAP: %s' % samname)
        return False    
